    ...
    this.io.on('connection', (socket: Socket) => {
      ...
      socket.on(
        ...
        async (data: {
          ...
          if (!data.sessionId || !data.clientType) {
            socket.emit('error', {
              message: 'Required connection parameters missing.',
            })
            return
          }
          ...
          try {
            ...
            if (!session) {
              socket.emit('error', {
                message: 'Chat session record not found.',
              })
              return
            }
            ...
            socket
              .to(roomName)
              .emit('presence_notification', systemNotification)
          } catch (error) {
            console.error('Error handling join_chat_session:', error)
            socket.emit('error', {
              message: 'Internal socket tracking error occurred.',
            })
          }
        },
      )
      ...
      // 4. REAL-TIME MULTI-TENANT MESSAGE PROCESSING PIPELINE (For Visitor Messages)
      socket.on(
        'send_message',
        async (data: {
          sessionId: string
          ...
          try {
            ...
            // Strict Security Check: Verify property isolation contexts match
            if (session.propertyId.toString() !== data.propertyId.toString()) {
              socket.emit('error', {
                message: 'Cross-site messaging packet interception blocked.',
              })
            ...
            // Deliver straight to all active interfaces in the conversation room
            this.io.to(roomName).emit('new_message', savedMessage)

            // If a visitor sent the message, alert the operator dashboard queue panel
            if (data.senderType === 'visitor') {
              const dashboardRoom = `property:dashboard:${data.propertyId}`
              this.io.to(dashboardRoom).emit('incoming_visitor_alert', {
                sessionId: data.sessionId,
