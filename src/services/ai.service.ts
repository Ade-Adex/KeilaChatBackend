...
//     message: string,
//     context: Partial<IMessage>[],
//     options?: { propertyId?: string; sessionId?: string },
//   ) {
//     try {
//       let sessionId = options?.sessionId || ''
//       let propertyId = options?.propertyId || ''

//       // Fallback: Resolve via history if options weren't passed directly
...
//       if (
//         intent === 'human' ||
//         humanWords.some((word) => cleanInput.includes(word))
//       ) {
//         const baseEscalationReply = randomResponse(escalationResponses)
//         return createResponse(
//           `${baseEscalationReply}\n\nWould you like me to transfer you to an agent? Please reply with "Yes" to confirm.`,
...
//       // Case B: Web content matches with sufficient confidence (>= 0.45)
...
//       /* HARD FALLBACK */
//       const finalConfidence = Math.max(faqConfidence, webConfidence)
//       const activeFallbackPhrase = getRandomFallback()
